...
# 1. LEXICOGRAPHICAL MINIMAL ENTERING/LEAVING
...
# 2. LEXICOGRAPHICAL MAXIMAL ENTERING/LEAVING
...
# 3. LARGEST COEFFICIENT

def largest_coefficient_entering(self):
    coef = list(DC.objective_coefficients())
    print coef
    var = list(DC.nonbasic_variables())
    print var
    dvar = list(D.nonbasic_variables())
    print dvar
    pvar = list(D.possible_entering())
    print pvar
    i = 0
    j = 0
    lce = None
    # Maximize:
    if m.objective.sense == -1:
        for x in var:
            if x not in pvar:
                coef[j] = -infinity
            if x not in dvar:
